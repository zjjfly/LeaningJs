...
console.log("Me too!");
//一般调用setTimeout的时候传入一个匿名函数
const timeId=setTimeout(function () {
    console.log("After timeout: " + new Date());
}, 3 * 1000);
clearTimeout(timeId);//和clearInterval类似，可以停止setTimeout的运行。
//还有一个和setTimeout类似的函数,setInterval,但不像setTimeout只执行一次,它会定时重复执行
...
//Node中，有一个error-first callback的习惯,就是callback的第一个参数是一个error对象,比如读取文件操作
...
//因为try catch只在同一个函数中起作用,但error是在回调的匿名函数中抛出的
